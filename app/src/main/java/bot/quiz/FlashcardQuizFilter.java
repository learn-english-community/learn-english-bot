...
public enum FlashcardQuizFilter {
    NEEDS_PRACTICE(
            ...
            "Practice with words that your spaced repetition profile thinks you should use.",
            Emoji.fromUnicode("💪"),
            (user, metadata) -> {
    ...
    WORD_QUALITY(
            "word-quality",
            "Word quality",
            "Select words based on the given quality.",
            Emoji.fromUnicode("✨"),
            (user, metadata) -> {
    ...
    PART_OF_SPEECH(
            "part-of-speech",
            "Part of speech",
            "Practice words based on their type.",
            Emoji.fromUnicode("💬"),
            (user, metadata) -> {
            ...
            () -> {
                ...
                TextInput textInput =
                        TextInput.create(
                                        "practice-input",
                                        "Enter the desired part of speech",
                                        TextInputStyle.SHORT)
    ...
    OLD(
            "old",
            "Old",
            "Practice words that you added a long time ago.",
            Emoji.fromUnicode("🕰️"),
            (user, metadata) -> {
                long now = System.currentTimeMillis();
                int range = Integer.parseInt(metadata.getAsString());
    ...
    ALL(
            "all",
            "All words",
            "Test yourself with your entire journal!",
            Emoji.fromUnicode("📘"),
    ...
    @Getter private final Supplier<Modal> metadataModal;

    @Getter private final BiFunction<User, ModalMapping, List<JournalWord>> filterProcessor;
